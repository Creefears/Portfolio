  ...
  useEffect(() => {
    const startAnimation = async () => {
      await controls.start({
        x: [0, -50 * duplicatedProjects.length],
        transition: {
          duration: duplicatedProjects.length * 5,
          ease: "linear",
          repeat: Infinity,
        }
      });
  ...
  return (
    <div className="absolute inset-0 overflow-hidden bg-gray-900">
      <motion.div 
        ...
        animate={controls}
        style={{ width: `${duplicatedProjects.length * 50}%` }}
      >
        {duplicatedProjects.map((project, index) => (
          <motion.div
            key={index}
            className="relative w-1/4 aspect-video cursor-pointer"
            whileHover={{ 
              scale: 1.1, 
              zIndex: 10,
